...
class authillo {
  ...
  public async begin(
    ...
    if (codeChallenge == null) {
      ...
      codeChallenge = await this._generateCodeChallenge().catch((reason) => {
        this.log(
          `failed to generate code challenge for following reason: ${reason}`
        );
        throw reason;
      });
    }
    ...
    this.log(
      "constucting url to redirect user to in order to begin authorization flow"
    );
    ...
    const redirectTo = `https://authillo.com/authorize?response_type=code&scope=openid${scopesString}&state=${state}&redirect_uri=${
      ...
      phoneNumberToAutoFill != null
        ? `&autofillPhoneNumber=${phoneNumberToAutoFill}`
        : ""
    }`;
    this.log(
      `redirecting the user to ${redirectTo} to begin authorization flow`
    );
  ...
  /**
   *
   * @description Function that identifies a user by a unique ID for account sharing detection
   * @param uniqueId - The unique ID of the user to identify. This can be any string that uniquely identifies the user. This could be a username, email, phone number, or any other unique identifier.
   */
  public identify = async (uniqueId: string) => {
    if (!this.initializationIsValid())
      throw `invalid configuration -- [make sure .initialize() is run before calling .begin()]`;
    this.log({ uniqueId });

    let deviceKey = localStorage.getItem("authillo-unique-id");
    if (deviceKey == null) {
      deviceKey = crypto.randomUUID();
      localStorage.setItem("authillo-unique-id", deviceKey);
    }
      ...
      async (event) => {
        ...
        const res = await fetch("http://localhost:3000/frontendjsidentify", {
          ...
          body: JSON.stringify({
            ...
            accountId: uniqueId,
            dataFromAuthillo: JSON.stringify(dataFromAuthillo),
            deviceKey,
          }),
        });
        this.log(res);
